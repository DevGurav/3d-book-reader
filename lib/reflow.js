...
function fontStr(px, weight) {
  return `${weight} ${px}px Georgia, "Times New Roman", serif`
}
 ...
 */
export function paginate(paragraphs, { fontPx, lineHeightMul, darkness }) {
...
export function renderReflowPage(lines, { fontPx, lineHeightMul, darkness, mode }) {
  const { W, H, margin, lineH: bodyLineH } = layout({ fontPx, lineHeightMul })
  ...
  for (const ln of lines || []) {
    if (ln && ln.text) {
      ctx.font = fontStr(ln.px ?? fontPx, ln.weight ?? (WEIGHT[darkness] ?? 600))
