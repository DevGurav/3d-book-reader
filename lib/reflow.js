...
function abortError() {
  const err = new Error('Reflow extraction canceled')
  err.name = 'AbortError'
  return err
}
 ...
 */
export async function extractParagraphs(pdf, { signal, onProgress } = {}) {
  const cached = paragraphCache.get(pdf)
  if (cached) return cached

  const extraction = extractParagraphsUncached(pdf, { signal, onProgress })
    .catch((err) => {
      paragraphCache.delete(pdf)
      throw err
    })

  paragraphCache.set(pdf, extraction)
  return extraction
}

async function extractParagraphsUncached(pdf, { signal, onProgress } = {}) {
  const paragraphs = []
...
function bodySize(paragraphs) {
  ...
  for (const p of paragraphs) {
    ...
    weighted.push({ size: p.size || 0, weight: n })
    totalWeight += n
  }
  if (!weighted.length) return 1
  weighted.sort((a, b) => a.size - b.size)
  const midpoint = Math.floor(totalWeight / 2)
  let seen = 0
  for (const item of weighted) {
    seen += item.weight
    if (seen > midpoint) return item.size || 1
  }
  return weighted[weighted.length - 1].size || 1
}
